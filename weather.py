...
def main():
    ...
    getInfo(page_soup)
    writeFile(name_of_file_one, name_of_file_two, name_of_file_three, page_soup)

def getInfo(locator):
    # prints out the region
    header = [header.text.strip() for header in locator.findAll('h1', {'class':'CurrentConditions--location--1Ayv3'})[0:]]
    for ent in header[0:]:
        print('\n\n' + ent)

    # prints out the time
    time = [time.text.strip() for time in locator.findAll('div', {'class':'CurrentConditions--timestamp--1SWy5'})[0:]]
    for tm in time[0:]:
        print(tm)
    ...
    for qualities in locator.findAll('div', {'class':'AirQuality--AirQualityCard--Ipx5M'})[0:]:
        idx, rtg, sty = qualities.find('text').text, qualities.find('span').text, qualities.find('p').text
        print('Level: ' + idx + ' = ' + rtg + '\nSeverity: ' + sty + '\n\n')
...
def checkFile(filename_one, filename_two, filename_three):
    try:
        if Path(filename_one).exists() and Path(filename_two).exists() and Path(filename_three):
            print('\n{' + filename_one + ' and ' + filename_two + ' and ' + filename_three + '} exists... Proceeding to Data Collection.')
    except IOError:
        print(IOError)
...
def writeFile(filename_one, filename_two, filename_three, locator):
    with open(filename_one, 'w') as f:
        ...
        for info in locator.findAll('div', {'class':'TodayWeatherCard--TableWrapper--13jpa'})[0:]:
            temps = [temps.text.strip().replace('°','') for temps in info.findAll('span', {'data-testid':'TemperatureValue'})[0:]]
    ...
    with open(filename_two, 'w') as f:
        ...
        for container in locator.findAll('div', {'class':'HourlyWeatherCard--TableWrapper--2kboH'})[0:]:
            time = [time.text.strip() for time in container.findAll('span', {'class':'Ellipsis--ellipsis--lfjoB'})[0:]]
            nw, st, nd, rd, th = time[0], time[1], time[2], time[3], time[4]
            
            tmp = [tmp.text.strip().replace('°', '') for tmp in container.findAll('span', {'data-testid':'TemperatureValue'})[0:]]
            nwC, stC, ndC, rdC, thC = tmp[0], tmp[1], tmp[2], tmp[3], tmp[4]
            
            rCh = [rCh.text.strip() for rCh in container.findAll('span', {'class':'Column--precip--2H5Iw'})[0:]]
            nwP, stP, ndP, rdP, thP = rCh[0], rCh[1], rCh[2], rCh[3], rCh[4]
            f.write(nw + ',' + st + ',' + nd + ',' + rd + ',' + th + '\n')
            f.write(nwC + 'C,' + stC + 'C,' + ndC + 'C,' + rdC + 'C,' + thC + 'C\n')
    ...
    with open(filename_three, 'w') as f:
        ...
        for container in locator.findAll('div', {'class':'DailyWeatherCard--TableWrapper--12r1N'})[0:]:
            dates = [dates.text.strip() for dates in container.findAll('span', {'class':'Ellipsis--ellipsis--lfjoB'})[0:]]
            td, dst, dnd, drd, dth = dates[0], dates[1], dates[2], dates[3], dates[4]
            
            high_temp = [high_temp.text.strip().replace('°', '') for high_temp in container.findAll('div', {'class':'Column--temp--2v_go'})[0:]]
            ttd, tst, tnd, trd, tth = high_temp[0], high_temp[1], high_temp[2], high_temp[3], high_temp[4]
            
            low_temp = [low_temp.text.strip().replace('°', '') for low_temp in container.findAll('div', {'class':'Column--tempLo--19O32'})[0:]]
            ltd, lst, lnd, lrd, lth = low_temp[0], low_temp[1], low_temp[2], low_temp[3], low_temp[4]
            
            prain = [prain.text.strip() for prain in container.findAll('div', {'class':'Column--precip--2H5Iw'})[0:]]
            ptd, pst, pnd, prd, pth = prain[0], prain[1], prain[2], prain[3], prain[4]            
            f.write(td + ',' + dst + ',' + dnd + ',' + drd + ',' + dth + '\n')
            f.write(ttd + 'C,' + tst + 'C,' + tnd + 'C,' + trd + 'C,' + tth + 'C\n')
            f.write(ltd + 'C,' + lst + 'C,' + lnd + 'C,' + lrd + 'C,' + lth + 'C\n')
